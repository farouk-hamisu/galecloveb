-- Update the lookup function to include email
...
-- Update the internal transfer function to return recipient's email
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 'Recipient not found'::TEXT, null::TEXT, null::TEXT, null::TEXT, null::TEXT;
    RETURN;
  END IF;

  -- Prevent self-transfer to the same account
  IF recipient_lookup_info.account_id = sender_account_info.id THEN
    RETURN QUERY SELECT false, 'Cannot transfer to the same account'::TEXT, null::TEXT, null::TEXT, null::TEXT, null::TEXT;
    RETURN;
  END IF;
  ...
  -- 7. Return success
  RETURN QUERY SELECT true, 'Transfer successful'::TEXT, v_reference_number, v_recipient_name, recipient_lookup_info.email, sender_account_info.currency;
